...
const Sidebar = ({ role, setView, currentView, onSearch, onPlaylistClick, onAvatarUpdate }) => {
  ...
  const handleAvatarUpload = async (e) => {
      ...
      try {
          ...
          // FIX: Trigger App to refresh image
  ...
  return (
    <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', height: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ marginBottom: '25px' }}>
        <input placeholder="Search..." onChange={(e) => onSearch(e.target.value)} style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: 'none', padding: '12px', borderRadius: '12px', color: 'white' }} />
      </div>
      ...
      <div className="desktop-only" style={{ marginTop: '40px', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '10px', marginBottom: '10px', color: '#64748b', fontSize: '11px', fontWeight: 'bold', display: 'flex', justifyContent: 'space-between' }}>
          LIBRARY <span onClick={createPlaylist} style={{ cursor: 'pointer', fontSize: '16px', color: '#fff' }}>+</span>
      </div>
      ...
      {role === 'admin' && <div className="desktop-only" style={{ marginTop: '10px' }}><div style={getStyle('users')} onClick={() => setView('users')}>👥 Users</div></div>}
    </div>
  );
};
